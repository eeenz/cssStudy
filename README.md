...
* `absolute` 설정 시 반드시 디자인에 따라 가족 관계 부모 중 하나에 `relative`를 설정하여 좌표값 `left, right, top, bottom` 중 1~2개를 작성해야 한다.
...
* `header`의 세로크기가 `height:100px`일 경우를 가정했을 때 nav의 `height:50px`인데 nav부터 붙어있게 하고 싶다면 `position:sticky; top:-50px;`로 입력한다.